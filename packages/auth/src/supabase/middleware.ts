...
/**
 * Updates session and returns the authenticated user
 * Useful for middleware that needs to check authentication
 *
 * @param request - Next.js request object
 * @returns Object with response and user
 *
 * @example
 * ```ts
 * import { updateSessionWithUser } from '@persepolis/auth/supabase/middleware'
 *
 * export async function middleware(request: NextRequest) {
 ...
 * ```
 */
export async function updateSessionWithUser(request: NextRequest) {
  let response = NextResponse.next({
    request: {
      headers: request.headers,
    },
